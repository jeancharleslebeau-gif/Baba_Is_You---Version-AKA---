#pragma once
#include <stdint.h>
#include "lib/graphics_basic.h"   // Compatibilité avec ton ancien système
...
-------------------------------------------------------------------------------
  Rôle :
    - Fournir une interface unique pour tout le moteur de jeu (sprites, texte,
      UI, animations, etc.).
    - Masquer les détails du backend (framebuffer ou LCD direct).
    - Permettre à BabaIsU et pAKAman de partager le même pipeline graphique.

  Backends :
    USE_FRAMEBUFFER = 1 → backend framebuffer (DMA, recommandé)
    USE_FRAMEBUFFER = 0 → backend direct LCD (pixel par pixel)

  Les backends sont implémentés dans :
    lib/gfx/gfx_fb.*      (framebuffer)
    lib/gfx/gfx_direct.*  (LCD direct)
...
// -----------------------------------------------------------------------------
// Fonction ajoutée pour BabaIsU : dessiner un sprite depuis un atlas
// -----------------------------------------------------------------------------

/*
    Dessine un sprite issu d’un atlas 16×16 ou autre.
...
// -----------------------------------------------------------------------------
// Affiche un bitmap RGB565 complet (w×h) à la position (dx, dy)
// -----------------------------------------------------------------------------
